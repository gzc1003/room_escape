		...
		function update(){
			...
			if(STAGE==3 && !window.HAS_PLAYED_JAZZ){
				if(STAGE==3 && CURRENT_LEVEL==1){
					var framesLeft = (rewindLevel.frames.length-rewindFrame) + levelObjects[5].frames.length;
					// TODO
					if(framesLeft<100){
...
function next(){
	...
	if(CURRENT_LEVEL<LEVEL_CONFIG.length){
		...
		startRewind();
		// TODO
		var totalFrames = levelObjects[0].frames.length
			+ levelObjects[1].frames.length
			+ levelObjects[2].frames.length
			+ levelObjects[3].frames.length
			+ levelObjects[4].frames.length
			+ levelObjects[5].frames.length;
...
function iMissYou(){
	...
	// TODO
	document.getElementById("canvas_container").style.backgroundPosition = "0px -780px ";
...
window.LEVEL_CONFIG = [
	...
	// M
	{
		...
		key:{ x:150, y:270 },
		circles: [
			{x:150,y:0,radius:125},
			{x:75,y:300,radius:75},
			{x:225,y:300,radius:75},
			{x:75,y:190,radius:55},
			{x:225,y:190,radius:55},
		],
		countdown:140
	},
	...
	// S
	{
		...
		key:{ x:85, y:110 },
		circles: [
			{x:25,y:170,radius:80},
			{x:275,y:130,radius:80},
			{x:150,y:75,radius:65},
			{x:150,y:225,radius:65},
		],
		countdown:130
	},
